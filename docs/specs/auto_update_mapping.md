...
## 📌 このドキュメントについて
このツールは、DiscordのVC参加者情報が記録されているCSVファイルを定期的にチェックし、新しいユーザーを発見したら自動的にDiscord-Slackマッピングシートに追加する自動化ツールです。手動でのマッピング作業を大幅に削減します。

## 🎯 機能概要
出勤記録のCSVファイルに新しいDiscordユーザーが記録されたとき、そのユーザーを自動的にマッピングシートに登録します。これにより、管理者は新規ユーザーのSlack IDのみを入力すれば良くなります。
...
| マッピングシート | Googleスプレッドシート | 既存のDiscord-Slackマッピング情報 | `discord_slack_mapping.xlsx` |
...
📋 マッピングシートの既存データを確認中...
マッピングシート発見: discord_slack_mapping.xlsx
