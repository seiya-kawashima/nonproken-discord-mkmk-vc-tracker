...
1. 画面上部のプロジェクトセレクタをクリック（現在のプロジェクト名が表示されている部分）
   - 初回の場合は「**プロジェクトを選択**」と表示されている

2. ポップアップウィンドウの右上にある「**新しいプロジェクト**」をクリック
   - もしくは、ダッシュボードに「**プロジェクトを作成**」ボタンがある場合はそれをクリック
