...
class DailyAggregator:
    ...
    def is_business_day(self, target_date: date) -> bool:
        ...
        # 祝日の判定（祝日も稼働日として扱う）
        # 注: 祝日判定機能は残すが、現在は祝日も稼働日として扱う
        # if jpholiday.is_holiday(target_date):  # 祝日
        #     return False

        return True  # 営業日（土曜日、祝日含む）
    ...
    def post_to_slack(self, user_data: Dict[str, Dict[str, Any]], stats_dict: Dict[str, Dict[str, Any]]) -> str:
        ...
        try:
            ...
            if user_data:
                ...
                # 参加者リストを1列形式で作成（ヘッダー付き）
                participants_lines = []
                participants_lines.append("*参加者 / 合計*")  # ヘッダー
                ...
                for user_id, data, total, consecutive in user_with_stats:
                    ...
                    if self.output_pattern == 'slack' and user_id in self.user_mapping:
                        ...
                        if slack_id.startswith('U'):
                            user_display = f"<@{slack_id}>さん"  # SlackユーザーID形式でメンション
                        else:
                            user_display = f"@{slack_id}さん"  # Slack表示名形式
                    else:
