...
def get_config(env: Environment = Environment.DEV) -> dict:
    ...
    return {
        ...
        'user_mapping_sheet_name': os.getenv(f'USER_MAPPING_SHEET_NAME_{suffix}', f'UserMapping_{suffix}'),  # DiscordユーザーIDとSlackメンションの対応表スプレッドシート名
        'user_mapping_sheet_id': os.getenv(f'USER_MAPPING_SHEET_ID_{suffix}'),  # ユーザーマッピングシートのID（直接指定する場合）
