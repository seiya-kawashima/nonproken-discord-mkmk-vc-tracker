...
class DriveCSVClient:
    ...
    def __init__(self, service_account_json: str, base_folder_path: str, env_suffix: str = "0_PRD", shared_drive_id: str = None, csv_path_template: str = None):
        ...
        # CSVパステンプレートを保存（configから渡された値を使用）
        self.csv_path_template = csv_path_template  # CSVファイルパステンプレート（configから渡される）
    ...
    def _ensure_csv_base_folder(self) -> str:
        ...
        Returns:
            CSVフォルダのID
        """
        # CSVパステンプレートからフォルダ名を抽出（configから渡されるパスを使用）
        # 例: "discord_mokumoku_tracker/csv/{vc_name}_0_PRD.csv" → "csv"
        path_parts = self.csv_path_template.split('/')  # パスを分割
        folder_name = path_parts[1] if len(path_parts) > 1 else 'csv'  # CSVフォルダ名を取得（パスの2番目の要素）
