...
from logger import logger  # テスト対象のloggerクラス
...
class TestLogger:
    ...
    def test_logger_initialization(self):
        """ロガーの初期化テスト"""
        test_logger = logger("test_module")  # テスト用ロガーを作成
    ...
    def test_debug_logging(self, caplog):
        """デバッグログのテスト"""
        test_logger = logger("test")  # テスト用ロガーを作成
    ...
    def test_file_handler_creation(self, mock_file_handler):
        """ファイルハンドラー作成のテスト"""
        test_logger = logger("test", log_file="test.log")  # ファイル出力付きロガーを作成
        mock_file_handler.assert_called()  # FileHandlerが呼ばれたか確認
    
    def test_logger_level_setting(self):
        """ログレベル設定のテスト"""
        test_logger = logger("test", level="WARNING")  # WARNINGレベルでロガーを作成
